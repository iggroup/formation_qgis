...
Préparer un projet **Biodiversite** pour le travail de terrain avec **QFieldSync**, puis exporter, transférer, utiliser et resynchroniser le projet sur QField.
...
1. Ouvrir QGIS.
2. Charger le fichier [biodiversite.qgs](../ressources/fichiers/base-de-donnees/biodiversite.qgs).
...
* les données doivent pointer vers le GeoPackage local `biodiversite_data.gpkg` ;
...
* Vérifier que les formulaires de saisie sont configurés pour les champs utiles.
* Ouvrir le projet dans QField pour windows et vérifier que les formulaires s'affichent correctement.

> **Note :** À ce stade, on a déjà un projet QGIS prêt à être utilisé dans QField, mais il n'est pas encore optimisé pour le terrain. L'étape suivante consiste à configurer les actions de synchronisation avec QFieldSync.
...
### Recommandation pour le cas Biodiversité
...
| `swissimage` (service **WMS**) | **Keep existing** (en ligne) ou **Basemap hors ligne générée** via QFieldSync | fond de plan visuel pour le repérage terrain |


* **Offline editing** : utile pour les couches que l’on veut modifier sur le terrain.
* **Copy** : utile pour les couches de référence à embarquer qui ne seront pas modifiées sur le terrain.
* **Keep existing** : utile si la couche ne change pas souvent et doit être recopiée seulement si nécessaire.
* **Basemap WMS (swissimage)** : un service WMS ne s'édite pas ; il est soit consommé en ligne, soit converti/embarquée en fond hors ligne pour le terrain.

Configuration recommandée pour `swissimage` :

1. si connexion disponible sur le terrain : garder le WMS en **Keep existing** ;
2. si travail hors ligne : générer un fond de plan offline depuis QFieldSync (emprise + niveaux de zoom) ;
3. limiter les niveaux de zoom utiles pour contenir la taille du paquet ;
4. vérifier l'affichage du fond en mode avion avant départ terrain.
...
1. vérifier la relation entre `observations` et `photos` ;

> **Point d'attention :** L'affichage swissimage peut être lent et consommer beaucoup de données. Il est souvent préférable de générer un fond hors ligne pour le terrain.
